...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "condition_type",
            sa.Enum("ENV", "THREAD_COUNT", "DURATION", "ENGINE_TYPE", "ALWAYS",
                    name="conditiontype"),
            nullable=False,
            comment="条件类型",
        ),
        sa.Column(
            "condition_op",
            sa.Enum("EQ", "NEQ", "GT", "GTE", "LT", "LTE", "IN",
                    name="conditionop"),
            nullable=False,
            server_default="EQ",
            comment="条件操作符",
        ...
        sa.Column("approver_roles", sa.JSON(), nullable=True, comment="审批角色列表 JSON"),
        sa.Column(
            "priority",
            sa.Integer(),
            nullable=False,
...
def downgrade() -> None:
    if not _table_exists("olh_approval_rule"):
        return
