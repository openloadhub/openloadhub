...
Use the good-first-issue template when proposing a starter task. Maintainers will add `good first issue` only after the work is narrow, public, and verifiable without private context; see [Issue Triage](docs/public/issue-triage.md).
...
When adding screenshots or logs to an issue or pull request, prefer the local demo URLs from [Quickstart](docs/public/quickstart.md), redact everything else, and include the OpenLoadHub commit plus Docker and browser versions.
